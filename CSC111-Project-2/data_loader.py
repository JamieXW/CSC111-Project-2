...
def load_apartment_data(csv_file: str) -> pd.DataFrame:
    ...
    """
    df = pd.read_csv(csv_file)
    df = df[df['Address'].str.lower().str.contains('toronto', na=False)] #filter the apartment data to only include toronto
    df['Price'] = (  #changing the price column values which are originally strings to floats
        df['Price']
        .str.replace('$', '', regex=False)  # Remove the dollar sign
        .str.replace(',', '', regex=False)  # Remove commas, if any
        .astype(float)  # Convert the cleaned string to float
    )
    return df
